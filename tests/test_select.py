...
async def test_select_change(hass: HomeAssistant, mock_configdata: dict):
    """Test Select change."""
    api_mock = APIMock()
    entry = MockConfigEntry(domain=DOMAIN, data=mock_configdata[DOMAIN])
    entry.add_to_hass(hass)
...
async def test_season_change(hass: HomeAssistant, mock_configdata: dict):
    """Test Season Select setup and Change."""
    api_mock = APIMock()
    entry = MockConfigEntry(domain=DOMAIN, data=mock_configdata[DOMAIN])
    entry.add_to_hass(hass)
